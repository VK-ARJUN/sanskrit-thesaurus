...
export const updateEntry = async (req, res) => {
  ...
  try {
    ...
    let updatedEntry = await Verb.findByIdAndUpdate(id, updatedData, {
      new: true,
    });

    if (!updatedEntry) {
      // If not found in Verb, try to update the entry in the Lookup collection
      updatedEntry = await Lookup.findByIdAndUpdate(id, updatedData, {
        new: true,
      });

      if (!updatedEntry) {
        // If not found in either collection, create a new entry in the Lookup collection
        updatedEntry = await Root.findByIdAndUpdate(id, updatedData, {
          new: true,
        });
        ...
        if (!updatedEntry) {
          ...
          return res
            .status(404)
            .json({
              error: "Entry not found in both Verb and Lookup collections",
            });
        }
      }
    }
